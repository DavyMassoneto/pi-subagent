...
test("registry warns on invalid descriptions by default and can be silenced", async () => {
  ...
  const def = new AgentRegistry();
  await def.reload(root, { discovery: { includeUserAgents: false }, onWarning: warning => defaultWarnings.push(warning) });
  assert.equal(def.agents.has("invalid"), false);
  assert.equal(defaultWarnings.length, 1);
  assert.match(defaultWarnings[0], /Invalid subagent definition.*Expected required field "description"/);

  const silentWarnings: string[] = [];
  const silent = new AgentRegistry();
  await silent.reload(root, {
    discovery: { warnOnInvalidAgents: false, includeUserAgents: false },
